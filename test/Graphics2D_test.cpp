...
{
  Image<unsigned char, 3> image(30, 30);
...
{
  Image<unsigned char, 3> image(30, 20);
...
void Graphics2D_test::test_drawQuadraticCurve()
{
  ...
  Image<unsigned char, 3> image(120, 100);

  gfx2d::Drawing::drawCurve(image, quadratic, {255, 0, 0});
  image.pixel(points[0][0], points[0][1]) = {255,255,255};
  image.pixel(points[1][0], points[1][1]) = {255,255,255};
...
void Graphics2D_test::test_drawCubicCurve()
{
  ...
  {
    image.pixel(point[0], point[1]) = {255,255,255};
  }

  // Split curve
  gfx2d::BezierCurve quartic = cubic.divide();

  // Draw curve and control points
  gfx2d::Drawing::drawCurve(image, quartic, {0, 255, 0});
  for (auto point : quartic.points())
  {
    image.pixel(point[0], point[1]) = {255,127,0};
  }
